#!/bin/bash
# unified-net-ssh-manager_v7_9.sh — 专业报告版（无 Emoji 报告 + 彩色高亮 + 版式修复）
set -euo pipefail
VERSION="v7.9"
REPORT_DIR="/root/net-ssh-report"; mkdir -p "$REPORT_DIR" 2>/dev/null || true
...
banner(){ echo "======================================================================"; printf "%b\n" "  ${BOLD}${CYAN}🧠  openEuler 智能桥接 + VLAN 与 SSH 动态转发 / SELinux 配置向导${NC}"; echo "======================================================================"; }
subtitle(){ echo "----------------------------------------------------------------------"; printf "%b\n" "  ${BOLD}${CYAN}$*${NC}"; echo "----------------------------------------------------------------------"; }
...
bridge_or_vlan_exists(){ ip link show type bridge >/dev/null 2>&1 && return 0; ip -d -o link show type vlan >/dev/null 2>&1 && return 0; return 1; }
...
#       lines+=$(printf "%b" "       ${DIM}UP：$(printf '%-12s' "$name") — $(printf '%-22s' "$ip_net")   gateway: $(printf '%-15s' "$gw")${NC}\n")
#     else
#       lines+=$(printf "%b" "       ${GREEN}${BOLD}UP：$(printf '%-12s' "$name") — $(printf '%-22s' "$ip_net")   gateway: $(printf '%-15s' "$gw")${NC}\n")
#     fi
...
build_iface_lines_block(){
  # 逐行输出，保留真实换行
  ip -o -4 addr show up 2>/dev/null | while read -r idx name fam addr rest; do
    local ip_net gw
    ip_net=$(echo "$addr" | awk '{print $1}')
    ...
    # lo 与虚拟设备灰色输出
    if [[ "$name" == "lo" || "$name" =~ ^(virbr|docker|tun|veth|br-|vlan|tap|wg) ]]; then
      printf "  ${DIM}UP：%-12s — %-22s   gateway: %-15s${NC}\n" "$name" "$ip_net" "$gw"
    else
      printf "  ${GREEN}${BOLD}UP：%-12s — %-22s   gateway: %-15s${NC}\n" "$name" "$ip_net" "$gw"
    fi
  done

  # # VLANs 概览行（左移 4 个空格）
  # local vlan_line
  # vlan_line="$(vlan_overview_line 2>/dev/null)"
  # printf "                %s\n" "$vlan_line"
}
...
get_iface_driver(){ have ethtool && tcmd 2 ethtool -i "$1" 2>/dev/null | awk -F': ' '/driver:/{print $2}' || echo "-"; }
...
print_iface_block_report(){
  local dev="$1"
  local ip4 ip6 mac mtu op car spd dup drv pci gw gwok rxp txp
  ...
  rxp="$(get_iface_rxpkts "$dev")"; txp="$(get_iface_txpkts "$dev")"

  local head_color="${GREEN}${BOLD}"
  [[ "$dev" =~ ^(virbr|docker|tun|veth|br-|vlan|tap|wg|lo)$ ]] && head_color="${DIM}"

  printf "%b\n" "${head_color}Interface : ${dev}${NC}"
  printf "State     : %-8s Carrier : %-8s MTU : %s\n" "$op" "$car" "$mtu"
  printf "Speed     : %-8s Duplex  : %s\n" "$spd" "$dup"
  printf "MAC       : %s\n" "$mac"
  printf "IPv4      : %s\n" "$ip4"
  printf "IPv6      : %s\n" "$ip6"
  if [ "$gwok" = "ok" ]; then printf "%b" "${GREEN}"; fi
  if [ "$gwok" = "fail" ]; then printf "%b" "${YELLOW}"; fi
  printf "Gateway   : %-16s Connectivity : %s\n" "$gw" "$gwok"
  printf "%b" "${NC}"
  printf "Driver    : %-14s PCI : %s\n" "$drv" "$pci"
  printf "RX/TX Pkts: %s / %s\n" "$rxp" "$txp"
  echo "----------------------------------------------------------------------"
}
...
print_vlan_report(){
  ...
  if ip -d -o link show type vlan >/dev/null 2>&1; then
    ...
    while read -r l; do
      ...
      printf "Name      : %-12s Parent : %-10s Tag : %-4s IPv4 : %s\n" "$name" "$parent" "$tag" "$ip4"
...
print_routes_report(){
  echo "路由概览"
  echo "----------------------------------------------------------------------"
  local def; def="$(ip route show default 2>/dev/null)"
  [ -n "$def" ] && printf "%b\n" "${GREEN}Default   : ${def}${NC}"
...
do_check_bridge_deep(){
  ...
  for dev in $(ip -o -4 addr show up | awk '{print $2}' | sort -u); do
    print_iface_block_report "$dev"
  done
  print_vlan_report
  print_routes_report
  local end=$(date +%s); printf "%b\n" "${DIM}（检测完成，用时 $((end-start)) 秒）${NC}"
}
...
collect_ssh_info(){
  ...
  if have ss; then
    SSH_LISTEN=$(ss -lntp 2>/dev/null | awk '/sshd/{print $4}' | paste -sd',' -)
  else
    SSH_LISTEN=""
  fi
}
...
do_check_ssh_deep(){
  ...
  collect_ssh_info
  printf "Service     : %s\n" "$SSH_SERVICE"
  printf "AllowForward: %s\n" "${SSH_ALLOW:-no}"
  printf "Port        : %s\n" "$SSH_PORTS_CONF"
  printf "Listen      : %s\n" "${SSH_LISTEN:-}"
  printf "PasswordAuth: %s\n" "${SSH_PASS_AUTH:-}"
  printf "PubkeyAuth  : %s\n" "${SSH_PUBKEY:-}"
  printf "PermitRoot  : %s\n" "${SSH_PERMIT_ROOT:-}"
  printf "GatewayPorts: %s\n" "${SSH_GATEWAY_PORTS:-}"
...
submenu_bridge(){
  ...
  if have nmcli && systemctl is-active --quiet NetworkManager; then
    printf "%b\n" "🌐 检测到 ${BOLD}NetworkManager 独占${NC}"
  else
    printf "%b\n" "🌐 检测到 ${BOLD}Legacy 网络${NC}"
  fi
  printf "%b\n\n" "${BOLD}当前接口状态：${NC}"
  build_iface_lines_block
  echo
  ...
  if bridge_and_ssh_ready; then
    printf "%b\n" "${CYAN}已检测到 Bridge/VLAN 与 SSH 转发均启用，将在 10 秒后自动进入综合深度检测...${NC}"
    ...
    if bridge_or_vlan_exists || ssh_forwarding_enabled; then
...
submenu_ssh(){
  ...
  local brief_allow=$(sshd -T 2>/dev/null | awk '/^allowtcpforwarding/{print $2}')
  local brief_ports=$(sshd -T 2>/dev/null | awk '/^port /{print $2}' | paste -sd',' -); [ -z "$brief_ports" ] && brief_ports="22"
  local brief_svc="inactive"; systemctl is-active --quiet sshd 2>/dev/null && brief_svc="active"
  echo "当前状态：$([ "${brief_allow:-no}" = "yes" ] && echo "已配置：TCP 转发启用（Port ${brief_ports}, service ${brief_svc}）" || echo "未配置：未启用 TCP 转发（Port ${brief_ports}, service ${brief_svc}）")"
  echo
...
submenu_check(){
  ...
  printf "%b" "${DIM}（按任意键返回上级菜单）${NC}"; read -rsn1 -t 0.1 _ || true; read -rsn1 -p "" _ || true; echo
}
...
main_menu(){
  ...
  if [ "$NET_MODE" = "nm" ]; then
    printf "%b\n" "🌐 网络管理由 ${BOLD}NetworkManager${NC} 独占。"
  else
    printf "%b\n" "🌐 当前为 ${BOLD}Legacy${NC} 网络模式。"
  fi
  printf "%b\n\n" "🔐 当前 SELinux 模式：${BOLD}${SE_MODE}${NC}（推荐：Permissive）"

  local VLLINE; VLLINE="$(vlan_overview_line)"

  echo "   [1] SELinux 模式修改"
  echo "   [2] Bridge + VLAN 配置（已配置接口："
  build_iface_lines_block | sed 's/^/       /'
  echo "         ${VLLINE}）"
  echo "   [3] SSH 动态转发与管理（$(ssh_forwarding_enabled && echo 已配置：TCP 转发启用 || echo 未配置：未启用 TCP 转发)）"
...
# 
#   echo "  📘 [1] SELinux 模式修改"
#   # printf "%b\n" "  📘 [2] Bridge + VLAN 配置（已配置接口："
#   # printf "%b"   "${IFBLOCK}\n"
#   # printf "%b\n" "       ${VLLINE}）"
#   echo "  📘 [2] Bridge + VLAN 配置（已配置接口："
#   echo -e "$IFBLOCK"        # ⚠️ 用 echo -e ，而不是 printf
#   echo "       ${VLLINE}）"
#   echo "  📘 [3] SSH 动态转发与管理（$(ssh_forwarding_enabled && echo 已配置：TCP 转发启用 || echo 未配置：未启用 TCP 转发)）"
#   echo "  📘 [4] 检查模式（Bridge + SSH 全面检测）"
#   echo "  📘 [5] 卸载 / 恢复"
